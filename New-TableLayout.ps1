    ...
    {#/¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯    ¯¯¯¯ -- ¯¯¯¯    ¯¯¯¯ -- ¯¯¯¯    ¯¯¯¯ -- ¯¯¯¯    ¯¯¯¯      
        ...
        Process
        {
            If ( $Function )
            {
                ...
                $Echo | % { $_.ST = 0..4 ; $_.FG = 0..4 ; $_.BG = 0..4 
                    ...
                    $_.ST[1] = @( " /" , $F[0] , "\$( $M1[70] )/" ;  $F  * 4 ; $F[0] , "\ " )              ; $_.FG[1] = @( @( 0 , 1 ) * 6 ; 0  )        ; $_.BG[1] = @( 3 ) * 13
                    $_.ST[2] = @( " \" , $F[1] , "/$( $M0[70] )\" ; @( $F[ 1 , 0 ] )  * 4 ; $F[1] , "/ " ) ; $_.FG[2] = @( 0 ; @( 1 ) * 11 ; 0 )        ; $_.BG[2] = @( 3 ) * 13
                    $_.ST[3] = @( "  ¯¯¯\" , "\__[ " , $FA , $Y ; @( $F ) * 4 ; "/¯¯¯  " )                 ; $_.FG[3] = @( 0 , 1 , 2 ; @( 1 , 0 ) * 5 ) ; $_.BG[3] = @( 3 ) * 13
            ...
            If ( $Table )
            {
                $Calc = 0 ; $List = @{ Class = @( ) ; ID = @( ) ; Section = @( ) ; Items = @( ) }

                $Table | GM | ? { $_.MemberType -eq "NoteProperty" } | % { $_.Name } | % { 

                    If ( $_ -like      "*ID*" ) { $List.ID      += $Table.$_ }
                    If ( $_ -like "*Section*" ) { $List.Section += $Table.$_ }
                }
                
                $List.Items += $List.Section.Keys

                $Calc   = $( ForEach ( $i in $List.Section ) { $Calc + 1 }
                             ForEach ( $i in $List.Items   ) { $Calc + 1 } )
                
                $Calc   = $Calc    | Measure -Sum | % { $_.Sum }
                
                If ( $Calc % 2 -ne 0 ) { $Calc = $Calc + 1 }    
                ...
                92 - $Title.Length | % { $Div = $_ / 2 ; $Rem = $_ % 2 ; $Sub = $Div % 2 ; $Add = "_" * $Sub ; $Z = 0 }

                $Output = $M1[$Div] + $Add + $Title + $M1[ ( $Div - $Sub ) ]

                $Echo | % {

                    $_.ST[$Z] = "  ____    $( $M1[100] )      "                                ; $_.FG[$Z] = 0 ;                         $_.BG[$Z] = 3           ; $Z++
                    $_.ST[$Z] = @( " /" ; $F ; "/$( $M0[98])\" , "\___  " )                    ; $_.FG[$Z] = 0 , 1 , 0 , 1 , 0 ;         $_.BG[$Z] = @( 3 ) * 5  ; $Z++
                    $_.ST[$Z] =    " \" , $F[1] , "/¯¯¯  " , $Output , "  ___/" , $F[0] , "\ " ; $_.FG[$Z] = 0 , 1 , 1 , 2 , 1 , 1 , 0 ; $_.BG[$Z] = @( 3 ) * 7  ; $Z++
                    $_.ST[$Z] = @( " //¯¯\" , "\$( $M1[98] )/" ; $F[ 0 , 1 ] ; "/ " )          ; $_.FG[$Z] = 0 , 1 , 0 , 1 , 0 ;         $_.BG[$Z] = @( 3 ) * 5  ; $Z++
                ...
                $Echo | % {
                    ...
                    $_.ST[$Z] = @( " \\___" ; $M2[72] ; @( "____" , "    " ) * 4 ; "___// " )               ; $_.FG[$Z] = @( 0 ) * 11                     ; $_.BG[$Z] = @( 3 ) * 11 ; $Z++
                    $_.ST[$Z] = @( " /" , $F[0] , "\$( $M1[70] )/" ; $F * 4 ; $F[0] , "\ " )                ; $_.FG[$Z] = @( @( 0 , 1 ) * 6 ; 0  )        ; $_.BG[$Z] = @( 3 ) * 13 ; $Z++
                    $_.ST[$Z] = @( " \" , $F[1] , "/$( $M0[70] )\" ; @( $F[ 1 , 0 ] )  * 4 ; $F[1] , "/ " ) ; $_.FG[$Z] = @( 0 ; @( 1 ) * 11 ; 0 )        ; $_.BG[$Z] = @( 3 ) * 13 ; $Z++
                    $_.ST[$Z] = @( "  ¯¯¯\" , "\$Return/" ; $F * 4 ; "/¯¯¯  " )                             ; $_.FG[$Z] = @( 0 , 1 , 2 ; @( 1 , 0 ) * 5 ) ; $_.BG[$Z] = @( 3 ) * 13 ; $Z++
        ...
        End
        {
            ...
            { 
                ...
                If ( $Count -eq 1 )
                {
                    Write-Host $Echo.ST[$I] -F $GX[$Echo.FG[$I]] -B $GX[$Echo.BG[$I]]
                }
